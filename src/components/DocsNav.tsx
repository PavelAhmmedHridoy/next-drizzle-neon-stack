...
export default function DocsNav() {
  ...
  const SocialLinks = () => (
    <div className="space-y-2 pt-4 border-t border-glass-border mt-4">
      <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="block px-3 py-2 rounded-lg text-sm font-medium text-foreground/70 hover:text-foreground hover:bg-accent/5 transition-all">GitHub</a>
      <a href="https://discord.com" target="_blank" rel="noopener noreferrer" className="block px-3 py-2 rounded-lg text-sm font-medium text-foreground/70 hover:text-foreground hover:bg-accent/5 transition-all">Discord</a>
    </div>
  )

  return (
    <>
      {/* Desktop Sidebar: Reduced from w-64 to w-56 */}
      <nav className="hidden lg:flex fixed left-0 top-0 h-screen w-56 bg-card border-r border-glass-border backdrop-blur-md flex-col p-6 z-40">
        <div className="mb-8">
          <Logo />
        </div>
      ...
      {/* Mobile Drawer: Reduced from w-72 to w-64 */}
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div
            ...
            <motion.div
              ...
              className="lg:hidden fixed left-0 top-0 h-screen w-64 bg-card border-r border-glass-border p-6 z-40 pt-20 flex flex-col justify-between"
            >
              <div className="flex-1 space-y-1 overflow-y-auto">
                {navItems.map((item) => (
                  <Link
                    ...
                    className={`block px-3 py-2 rounded-lg transition-all text-sm font-medium ${
                      pathname === item.href ? 'bg-accent/10 text-accent' : 'text-foreground/70'
                    }`}
                  >
                    {item.label}
                  </Link>
